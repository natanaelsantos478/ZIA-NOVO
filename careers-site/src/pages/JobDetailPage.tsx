...
import type { Vacancy, DesiredProfiles, CandidatePayload } from '../types/vacancy';
import { getVacancyBySlug, submitCandidate } from '../services/vacanciesService';
...
interface ProfileQuestion {
  ...
  text: string;
  options: { label: string; value: keyof DesiredProfiles }[];
}
...
function ProfileTest({ onComplete }: { onComplete: (results: Partial<DesiredProfiles>) => void }) {
  const [answers, setAnswers] = useState<Record<string, keyof DesiredProfiles>>({});
  const allAnswered = PROFILE_QUESTIONS.every((q) => answers[q.id]);

  function submit() {
    const counts: Record<keyof DesiredProfiles, number> = { executor: 0, analitico: 0, comunicador: 0, planejador: 0 };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start gap-3 bg-purple-50 border border-purple-200 rounded-xl p-4">
        ...
        <div>
          ...
          <p className="text-xs text-purple-600 mt-0.5">
            4 perguntas rápidas. Não há resposta certa — queremos conhecer seu estilo de trabalho.
          </p>
        </div>
      </div>
      {PROFILE_QUESTIONS.map((q, i) => (
        <div key={q.id}>
          <p className="text-sm font-semibold text-slate-700 mb-3">{i + 1}. {q.text}</p>
      ...
      <button
        ...
        className="w-full py-3 text-sm font-semibold text-white bg-purple-600 rounded-xl hover:bg-purple-700 disabled:opacity-40 disabled:cursor-not-allowed"
      >
        Confirmar respostas
      </button>
    </div>
...
export default function JobDetailPage() {
  ...
  const [source, setSource] = useState<CandidatePayload['source']>('portal');
  const [profileAnswers, setProfileAnswers] = useState<Partial<DesiredProfiles>>({});
  const [profileDone, setProfileDone] = useState(false);

  useEffect(() => {
    if (!slug) return;
    getVacancyBySlug(slug)
      .then((v) => setVacancy(v ?? null))
  ...
  async function handleSubmit() {
    ...
    try {
      await submitCandidate({
        ...
        name, email, phone, linkedin, message, source,
        profileAnswers,
      });
      setSubmitted(true);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center gap-3 text-slate-400">
        ...
        <span className="text-sm">Carregando vaga...</span>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-6 px-6">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-10 max-w-md w-full text-center">
          ...
          <p className="text-sm text-slate-500 mb-1">Obrigado, <strong>{name}</strong>!</p>
          <p className="text-sm text-slate-500 mb-6">
            Recebemos sua candidatura para <strong>{vacancy.title}</strong>. Nossa equipe de RH entrará em contato em breve pelo e-mail <strong>{email}</strong>.
          </p>
          {Object.values(profileAnswers).some(Boolean) && (
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 mb-6 text-left">
              ...
              <div className="flex flex-wrap gap-2">
                {(Object.entries(profileAnswers) as [keyof DesiredProfiles, boolean][])
                  .filter(([, v]) => v)
                  .map(([k]) => (
  ...
  const canSubmit = name && email && phone &&
    (!vacancy.requireProfileAnalysis || profileDone);
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${MODALITY_COLOR[vacancy.modality]}`}>{vacancy.modality}</span>
              <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${TYPE_COLOR[vacancy.type]}`}>{vacancy.type}</span>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 sticky top-20">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="tel" placeholder="(11) 99999-9999" value={phone} onChange={(e) => setPhone(e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">LinkedIn (opcional)</label>
                <input type="url" placeholder="linkedin.com/in/seu-perfil" value={linkedin} onChange={(e) => setLinkedin(e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">Mensagem (opcional)</label>
                <textarea rows={3} placeholder="Por que você tem interesse nesta vaga?" value={message} onChange={(e) => setMessage(e.target.value)} className={`${inputCls} resize-none`} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">Como ficou sabendo desta vaga?</label>
                <select value={source} onChange={(e) => setSource(e.target.value as CandidatePayload['source'])} className={`${inputCls} bg-white`}>
              ...
              {vacancy.requireProfileAnalysis && !profileDone && (
                <div className="border-t border-slate-100 pt-4">
                  <ProfileTest onComplete={(r) => { setProfileAnswers(r); setProfileDone(true); }} />
                </div>
              )}

              {profileDone && (
                <div className="flex items-center gap-2 bg-purple-50 border border-purple-200 rounded-xl px-3 py-2 text-xs text-purple-700">
                  <CheckCircle className="w-3.5 h-3.5 shrink-0" /> Análise de perfil concluída
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3 text-sm font-bold text-white bg-pink-600 rounded-xl hover:bg-pink-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
