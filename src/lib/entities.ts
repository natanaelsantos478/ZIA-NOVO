...
// ─────────────────────────────────────────────────────────────────────────────
import { supabase, getFilialDb } from './supabase';
...
export async function getClientes(search = ''): Promise<Cliente[]> {
  let q = getFilialDb().from('erp_clientes').select('*').order('nome');
...
export async function createCliente(payload: Omit<Cliente, 'id' | 'tenant_id' | 'created_at'>): Promise<Cliente> {
  const { data, error } = await getFilialDb().from('erp_clientes').insert({ ...payload, tenant_id: getFilialId() }).select().single();
  if (error) throw error;
  return data;
}

export async function updateCliente(id: string, payload: Partial<Cliente>): Promise<Cliente> {
  const { data, error } = await getFilialDb().from('erp_clientes').update(payload).eq('id', id).select().single();
  if (error) throw error;
  return data;
}

export async function deleteCliente(id: string): Promise<void> {
  const { error } = await getFilialDb().from('erp_clientes').delete().eq('id', id);
  if (error) throw error;
}
...
export async function getFornecedores(search = ''): Promise<Fornecedor[]> {
  let q = getFilialDb().from('erp_fornecedores').select('*').order('nome');
  if (search) q = q.ilike('nome', `%${search}%`);
  const { data, error } = await q;
  if (error) throw error;
  return data ?? [];
...
export async function createFornecedor(payload: Omit<Fornecedor, 'id' | 'tenant_id' | 'created_at'>): Promise<Fornecedor> {
  const { data, error } = await getFilialDb().from('erp_fornecedores').insert({ ...payload, tenant_id: getFilialId() }).select().single();
  if (error) throw error;
  return data;
}

export async function updateFornecedor(id: string, payload: Partial<Fornecedor>): Promise<Fornecedor> {
  const { data, error } = await getFilialDb().from('erp_fornecedores').update(payload).eq('id', id).select().single();
  if (error) throw error;
  return data;
}

export async function deleteFornecedor(id: string): Promise<void> {
  const { error } = await getFilialDb().from('erp_fornecedores').delete().eq('id', id);
  if (error) throw error;
}
...
export async function getGruposProdutos(): Promise<GrupoProduto[]> {
  const { data, error } = await getFilialDb().from('erp_grupo_produtos').select('*').order('nome');
  if (error) throw error;
  return data ?? [];
}

export async function createGrupoProduto(payload: Omit<GrupoProduto, 'id' | 'tenant_id' | 'created_at'>): Promise<GrupoProduto> {
  const { data, error } = await getFilialDb().from('erp_grupo_produtos').insert({ ...payload, tenant_id: getFilialId() }).select().single();
  if (error) throw error;
  return data;
}

export async function updateGrupoProduto(id: string, payload: Partial<GrupoProduto>): Promise<GrupoProduto> {
  const { data, error } = await getFilialDb().from('erp_grupo_produtos').update(payload).eq('id', id).select().single();
  if (error) throw error;
  return data;
}

export async function deleteGrupoProduto(id: string): Promise<void> {
  const { error } = await getFilialDb().from('erp_grupo_produtos').delete().eq('id', id);
  if (error) throw error;
}
...
export async function getProdutos(search = ''): Promise<Produto[]> {
  let q = getFilialDb().from('erp_produtos').select('*, erp_grupo_produtos(nome)').order('nome');
  if (search) q = q.ilike('nome', `%${search}%`);
  const { data, error } = await q;
  if (error) throw error;
  return data ?? [];
...
export async function createProduto(payload: Omit<Produto, 'id' | 'tenant_id' | 'created_at' | 'estoque_atual' | 'erp_grupo_produtos'>): Promise<Produto> {
  const { data, error } = await getFilialDb().from('erp_produtos').insert({ ...payload, tenant_id: getFilialId(), estoque_atual: 0 }).select().single();
  if (error) throw error;
  return data;
}

export async function updateProduto(id: string, payload: Partial<Produto>): Promise<Produto> {
  const { data, error } = await getFilialDb().from('erp_produtos').update(payload).eq('id', id).select().single();
  if (error) throw error;
  return data;
}

export async function deleteProduto(id: string): Promise<void> {
  const { error } = await getFilialDb().from('erp_produtos').delete().eq('id', id);
  if (error) throw error;
}
...
export async function getAtividades(modulo?: string): Promise<Atividade[]> {
  let q = getFilialDb().from('erp_atividades').select('*').order('created_at', { ascending: false });
...
export async function createAtividade(payload: Omit<Atividade, 'id' | 'tenant_id' | 'created_at'>): Promise<Atividade> {
  const { data, error } = await getFilialDb().from('erp_atividades').insert({ ...payload, tenant_id: getFilialId() }).select().single();
  if (error) throw error;
  return data;
}
...
export async function updateAtividadeStatus(id: string, status: Atividade['status']): Promise<void> {
  ...
  const { error } = await getFilialDb().from('erp_atividades').update(update).eq('id', id);
  if (error) throw error;
}
...
// supabase re-export para uso direto (CompanySelector, orgStructure etc.)
export { supabase };
