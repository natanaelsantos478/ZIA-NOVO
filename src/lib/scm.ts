...
export async function getScmKpis(): Promise<ScmKpis> {
  const [
    shipmentsRes, deliveriesRes, vehiclesRes, driversRes,
    auditsRes, returnsRes, freightSpendRes, discrepancyRes,
  ] = await Promise.all([
  ...
  return {
    ...
    pendingDeliveries: deliveries.filter((d) => ['pending', 'out_for_delivery'].includes(d.status)).length,
    availableVehicles: vehicles.filter((v) => v.status === 'available').length,
    totalVehicles:     vehicles.length,
    activeDrivers:     drivers.filter((d) => ['available', 'on_route'].includes(d.status)).length,
    pendingAudits:     audits.filter((a) => ['pending', 'disputed'].includes(a.status)).length,
...
export async function getDrivers(): Promise<Driver[]> {
  const { data, error } = await supabase.from('scm_drivers').select('*').order('name');
  if (error) throw error;
  return (data ?? []) as Driver[];
}

export async function createDriver(payload: Partial<Driver>): Promise<Driver> {
  const { data, error } = await supabase.from('scm_drivers').insert(payload).select().single();
  if (error) throw error;
  return data as Driver;
}
...
export async function getVehicles(): Promise<Vehicle[]> {
  const { data, error } = await supabase
    .from('scm_vehicles').select('*, scm_drivers(name)').order('plate');
  if (error) throw error;
...
export async function createVehicle(payload: Partial<Vehicle>): Promise<Vehicle> {
  const { data, error } = await supabase
    .from('scm_vehicles').insert(payload).select('*, scm_drivers(name)').single();
  if (error) throw error;
  return data as Vehicle;
}
...
export async function createRoute(
  payload: Omit<Route, 'id' | 'created_at' | 'scm_vehicles' | 'scm_drivers'>
): Promise<Route> {
  const { data, error } = await supabase
    .from('scm_routes')
    .insert(payload)
...
export async function createDockSession(payload: Partial<DockSession>): Promise<DockSession> {
  const { data, error } = await supabase
    .from('scm_dock_sessions').insert(payload).select().single();
  if (error) throw error;
  return data as DockSession;
}
...
export async function createReverseLogistic(
  payload: Partial<ReverseLogistic>
): Promise<ReverseLogistic> {
  const { data, error } = await supabase
    .from('scm_reverse_logistics')
    .insert(payload)
    .select('*, scm_shipments(code)')
...
export async function updateReverseLogistic(
  id: string, payload: Partial<ReverseLogistic>
): Promise<void> {
...
export async function updateFreightAudit(
  id: string, payload: Partial<FreightAudit>
): Promise<void> {
