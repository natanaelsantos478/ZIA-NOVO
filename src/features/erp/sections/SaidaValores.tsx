...
import {
  ArrowUpCircle, Loader2, CheckCircle, AlertCircle, X, Plus,
  Search, Filter, Download, Clock, TrendingUp, Calendar,
} from 'lucide-react';
import {
...
const diasAtraso = (venc: string) => Math.floor((new Date().getTime() - new Date(venc + 'T00:00').getTime()) / 86400000);
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div>
            <h2 className="font-bold text-slate-900">Baixar Pagamento</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" min="0" step="0.01" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Conta de Saída</label>
            <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
          ...
          <div>
            ...
            <input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
              value={numDoc} onChange={e => setNumDoc(e.target.value)} placeholder="Nº boleto, comprovante PIX..." />
          </div>
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          <button onClick={handlePagar} disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white py-2.5 rounded-lg text-sm font-medium">
            {saving && <Loader2 className="w-4 h-4 animate-spin" />} Confirmar Pagamento
          </button>
        </div>
      </div>
    </div>
...
function ModalParcelar({
  ...
  function updateParcelas(n: number) {
    ...
    setParcelas(Array.from({ length: n }, (_, i) => {
      const d = new Date(); d.setMonth(d.getMonth() + i + 1);
      return {
        valor: i === n - 1 ? (lancamento.valor - valUnit * (n - 1)).toFixed(2) : valUnit.toFixed(2),
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h2 className="font-bold text-slate-900">Parcelar Despesa</h2>
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={onClose} className="flex-1 border border-slate-200 text-slate-600 py-2.5 rounded-lg text-sm">Cancelar</button>
...
export default function SaidaValores() {
  ...
  const [modalPagar, setModalPagar] = useState<ErpLancamento | null>(null);
  ...
  async function reload() {
    const [l, c] = await Promise.all([getLancamentos('DESPESA'), getContasBancarias()]);
    setLancamentos(l); setContas(c);
  }
  ...
  async function handleSave() {
    ...
    try {
      await createLancamento({
        tipo: 'DESPESA', categoria, descricao, valor: +valor,
        data_vencimento: dataVencimento,
        data_pagamento: marcarPago ? today() : null,
  ...
  async function handleCancelar() {
    ...
    try {
      ...
      setCancelando(null); setJustificativa('');
      reload();
    } catch (e) { showToast('Erro: ' + (e as Error).message, false); }
  }
  ...
  const pagoCusto = lancamentos.filter(l => l.status === 'PAGO' && (l.data_pagamento ?? '').startsWith(mesAtual)).reduce((s, l) => s + (l.valor_pago ?? l.valor), 0);
  ...
  const filtrados = useMemo(() => {
    ...
    return list;
  }, [lancamentos, filtroStatus, filtroCategoria, filtroPeriodo, filtroSearch]);

  function exportCSV() {
    const header = 'Descrição,Categoria,Vencimento,Valor,Status,Forma Pag.,Documento';
    const rows = filtrados.map(l =>
      `"${l.descricao}",${l.categoria},${l.data_vencimento},${l.valor},${l.status},${l.forma_pagamento ?? ''},${l.numero_documento ?? ''}`
    );
  ...
  const statusBadge = (l: ErpLancamento) => {
    if (l.status === 'PAGO') return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-600">PAGO</span>;
    if (l.status === 'CANCELADO') return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-400">CANCELADO</span>;
  ...
  return (
    <div className="p-6">
      ...
      {modalPagar && (
        <ModalPagar lancamento={modalPagar} contas={contas} onClose={() => setModalPagar(null)}
          onSuccess={() => { setModalPagar(null); showToast('Pagamento registrado.', true); reload(); }} />
      )}
      {modalParcelar && (
        <ModalParcelar lancamento={modalParcelar} onClose={() => setModalParcelar(null)}
          onSuccess={() => { setModalParcelar(null); showToast('Despesa parcelada.', true); reload(); }} />
      )}
      {cancelando && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Gerencie despesas, fornecedores e obrigações financeiras</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <div className="flex items-center gap-2 mb-2"><Clock className="w-4 h-4 text-amber-500" /><span className="text-xs text-slate-500">A Pagar</span></div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <div className="flex items-center gap-2 mb-2"><TrendingUp className="w-4 h-4 text-slate-500" /><span className="text-xs text-slate-500">Pago no Mês</span></div>
          <div className="text-xl font-bold text-slate-700">{fmtBRL(pagoCusto)}</div>
        </div>
        <div className="bg-white rounded-xl border border-orange-100 p-4">
          <div className="flex items-center gap-2 mb-2"><Calendar className="w-4 h-4 text-orange-500" /><span className="text-xs text-slate-500">Previsão 30 dias</span></div>
          <div className="text-xl font-bold text-orange-600">{fmtBRL(previsao30d)}</div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
                value={descricao} onChange={e => setDescricao(e.target.value)} placeholder="Ex: Aluguel, energia, fornecedor..." />
            </div>
            <div>
            ...
            <div className="col-span-2 flex items-center gap-3">
              <input type="checkbox" id="marcoPago" checked={marcarPago} onChange={e => setMarcarPago(e.target.checked)} className="rounded" />
              <label htmlFor="marcoPago" className="text-sm text-slate-700">Já pago</label>
            </div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
        ...
        <div className="flex items-center gap-1.5">
          ...
          {['TODOS', 'PENDENTE', 'VENCIDO', 'PAGO', 'CANCELADO'].map(s => (
            <button key={s} onClick={() => setFiltroStatus(s)}
              className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors ${filtroStatus === s ? 'bg-red-600 text-white border-red-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
              {s}
            </button>
          ))}
        </div>
              ...
              <tr key={l.id} className={`hover:bg-slate-50 transition-colors ${l.status === 'PENDENTE' && l.data_vencimento < hoje ? 'bg-red-50/30' : ''}`}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    {l.status === 'PENDENTE' && (
                      <>
                        <button onClick={() => setModalPagar(l)}
                          className="text-xs text-white bg-red-600 hover:bg-red-700 px-2.5 py-1 rounded font-medium transition-colors">
                          Pagar
                        </button>
