...
export default function PedidoVenda() {
  ...
  return (
    <div className="fixed top-16 inset-x-0 bottom-0 z-40 flex">
      {toast && (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-2.5 rounded-lg shadow-lg text-sm text-white ${toast.ok ? 'bg-green-600' : 'bg-red-600'}`}>
          {toast.ok ? <CheckCircle className="w-4 h-4 flex-shrink-0" /> : <AlertCircle className="w-4 h-4 flex-shrink-0" />}
          {toast.msg}
      ...
      <div className="w-48 bg-slate-800 flex flex-col flex-shrink-0 overflow-y-auto">
          ...
          <div className="p-5">
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              ...
              <div className="col-span-2 relative">
                ...
                {clienteSelecionado ? (
                  /* Cliente selecionado — exibe card com botão de trocar */
                  <div className="flex items-center gap-2 border border-green-300 bg-green-50 rounded-lg px-3 py-2">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-800 truncate">{clienteSelecionado.nome}</p>
                      <p className="text-xs text-slate-500">{clienteSelecionado.cpf_cnpj} · {clienteSelecionado.tipo}</p>
                    </div>
                    <button
                  ...
                  <div className="relative">
                    ...
                    {showClienteList && clienteSearch && (
                      <div className="absolute z-20 top-full left-0 right-0 bg-white border border-slate-200 rounded-lg shadow-lg max-h-48 overflow-y-auto mt-0.5">
                        {clientesFiltrados.length === 0 ? (
                          <p className="px-3 py-3 text-xs text-slate-400 text-center">Nenhum cliente encontrado para "{clienteSearch}"</p>
                          ...
                          >
                            ...
                            <span className="ml-2 text-slate-400">{c.cpf_cnpj}</span>
                            <span className={`ml-2 px-1.5 py-0.5 rounded text-[10px] font-medium ${c.tipo === 'PJ' ? 'bg-blue-100 text-blue-600' : 'bg-purple-100 text-purple-600'}`}>{c.tipo}</span>
                          </button>
                        ))}
                      </div>
                    )}
