...
import { ArrowRightLeft, RefreshCw, AlertTriangle, Search } from 'lucide-react';
...
export default function SCMCrossDock() {
  ...
  useEffect(() => {
    fetchDockSessions()
      .then(data => setSessions(data.filter(s => s.type === 'crossdock')))
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-2 text-slate-500">Carregando cross-docking...</span>
      </div>
    );
  }
  ...
  const filtered = sessions.filter(s =>
    [s.vehicle_plate, s.carrier, String(s.dock_number)].some(f =>
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">{sessions.length} operação(ões) de cross-docking</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-slate-400">
          ...
          <p>{search ? 'Nenhuma operação encontrada.' : 'Nenhuma operação de cross-docking registrada.'}</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-semibold text-slate-600">Concluído em</th>
