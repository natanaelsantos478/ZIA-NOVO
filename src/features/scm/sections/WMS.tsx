...
interface DockForm {
  dock_number: string; vehicle_plate: string; carrier: string;
  type: string; status: string; scheduled_at: string;
  pallet_count: string; notes: string;
}
...
export default function WMS() {
  ...
  function openEdit(s: DockSession) {
    ...
    setForm({
      ...
      vehicle_plate: s.vehicle_plate ?? '', carrier: s.carrier ?? '',
      type: s.type, status: s.status,
      scheduled_at: s.scheduled_at ? s.scheduled_at.slice(0, 16) : '',
      pallet_count: s.pallet_count?.toString() ?? '', notes: s.notes ?? '',
    });
    setShowModal(true);
  }
  ...
  async function handleSave() {
    ...
    try {
      const payload: Partial<DockSession> = {
        ...
        vehicle_plate: form.vehicle_plate || null, carrier: form.carrier || null,
