...
import { Settings, LogOut, LayoutGrid, Bell, RefreshCw, Building2, Shield, Users, User } from 'lucide-react';
    ...
    <header className={`h-16 ${headerBg} text-white flex items-center justify-between px-6 shadow-md z-50 relative shrink-0`}>
      ...
      <div className="flex items-center space-x-6">
        ...
        <div className="relative">
          ...
          {/* Menu Dropdown */}
          {profileOpen && (
            <div className="absolute right-0 mt-3 w-56 bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right z-50">
              {activeProfile && (
                <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-indigo-100 flex items-center justify-center">
                      <ProfileIcon className="w-4 h-4 text-indigo-600" />
                    </div>
                    <div>
                  ...
                  <div className="mt-2 text-[10px] font-mono text-slate-400">Código: #{activeProfile.code}</div>
                  <div className="text-[10px] text-slate-400 truncate">{activeProfile.entityName}</div>
                </div>
              )}
              <div className="p-1">
                <button
                  onClick={handleSwitchProfile}
                  className="w-full flex items-center px-3 py-2 text-sm text-slate-600 hover:bg-amber-50 hover:text-amber-700 rounded-lg transition-colors"
                >
